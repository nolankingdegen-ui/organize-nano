...
- However, make sure you never wait for logs to show up in expo.log file since it is on runtime and requires you to respond to the user before anything shows up in the logs file.
  - Only use console.error() for errors that are breaking the app since console.error() shows up as an error in the Vibecode app. If there are non-breaking errors, use console.log() instead.
